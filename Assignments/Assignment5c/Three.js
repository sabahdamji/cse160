...
function main() {
  ...
  const banana2 = new THREE.Mesh(bananaGeometry, bananaMaterial);
  banana2.position.set(6, 5, -9); 
  banana2.rotation.x = 3;
  banana2.rotation.y = 2;
  scene.add(banana2);

  const banana3 = new THREE.Mesh(bananaGeometry, bananaMaterial);
  banana3.position.set(12, 5, 1); 
  banana3.rotation.x = 3;
  banana3.rotation.y = 2;
  scene.add(banana3);
  ...
  function render(time) {
    ...
    banana2.rotation.x += 0.01;
    banana2.rotation.y += 0.01;

    banana3.rotation.x += 0.01;
    banana3.rotation.y += 0.01;

    if (monkey) {
      const jumpHeight = 0.5; 
      const jumpSpeed = 2; 
      const yPos = Math.sin(time * jumpSpeed) * jumpHeight;
      monkey.position.y = yPos + 0.5;
  
      // make monkey dance!
      const danceSpeed = 5; 
      const danceAmount = Math.sin(time * danceSpeed) * 2.5; 
      monkey.rotation.z = danceAmount;
  }
